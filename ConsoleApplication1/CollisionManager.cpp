...
{
	int vecX = collider1->GetPositionX() - collider2->GetPositionX();
	int vecY = collider1->GetPositionY() - collider2->GetPositionY();
	int squarelength = vecX * vecX + vecY * vecY;
	int radiuses = collider1->GetRadius() + collider2->GetRadius();
	radiuses *= radiuses;
	return squarelength < radiuses;
}
